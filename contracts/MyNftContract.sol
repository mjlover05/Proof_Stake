...
contract MyNFTContract is ERC721 {
    ...
    function mintNFT(address recipient, string memory _tokenURI) public returns (uint256) {
        require(msg.sender == contractOwner, "Only contract owner can mint NFTs");
    ...
    function batchMint(address[] memory recipients, string[] memory tokenURIsArray) public returns (uint256[] memory) {
        require(recipients.length == tokenURIsArray.length, "BatchMint: Arrays length mismatch");
